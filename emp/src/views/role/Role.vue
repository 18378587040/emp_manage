<template>
  <el-card class="main-card">
    ...
    <el-dialog :visible.sync="roleMenu" width="30%">
      ...
      <el-form label-width="80px" size="medium" :model="roleForm">
        <el-form-item label="角色名">
          <el-input placeholder="请输入角色英文名" v-model="roleForm.roleName" style="width:250px" >
          </el-input>
        </el-form-item>
        <el-form-item label="权限标签">
          <el-input placeholder="请输入角色中文名" v-model="roleForm.roleLabel" style="width:250px" />
        </el-form-item>
    ...
    <el-dialog :visible.sync="roleResource" width="30%" top="9vh">
      ...
      <el-form label-width="80px" size="medium" :model="roleForm">
        <el-form-item label="角色名">
          <el-input v-model="roleForm.roleName" placeholder="请输入角色英文名" style="width:250px" >
          </el-input>
        </el-form-item>
        <el-form-item label="权限标签">
          <el-input v-model="roleForm.roleLabel" placeholder="请输入角色中文名" style="width:250px" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    listRoles() {
        ...
        .then((data) => {
          this.roleList = data.recordList;
          this.count = data.count;
          this.loading = false;
        });
      this.getRequest("/api/admin/role/resources").then((data) => {
        this.resourceList = data;
      });
      this.getRequest("/api/admin/role/menus").then((data) => {
        this.menuList = data;
      });
    },
    deleteRoles(id) {
    ...
    saveOrUpdateRoleResource() {
      ...
      this.postRequest("/api/admin/role", this.roleForm).then((data) => {
        if (data) {
          this.$notify.success({
            title: "成功",
            message: "操作成功"
